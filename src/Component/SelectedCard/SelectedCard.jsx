const SelectedCard = ({ cart, handleRemove, handleCheckout }) => {
    ...
    return (
        <div className="max-w-3xl mx-auto py-10 animate-fade-in">
            <h2 className="text-3xl font-black mb-10 text-center text-gray-800 underline underline-offset-8 decoration-[#4F39F6]">Your Shopping Cart</h2>
            ...
            {cart.length === 0 ? (
                ...
                <div className="space-y-6">
                    ...
                    {/* Order Summary */}
                    <div className="bg-white p-10 rounded-[40px] shadow-2xl border-t-8 border-[#4F39F6] mt-12">
                        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                            <div className="text-center md:text-left">
                                <p className="text-gray-500 font-bold uppercase tracking-widest text-xs mb-1">Total Products: {cart.length}</p>
                                <h3 className="text-5xl font-black text-gray-900">Total: <span className="text-[#4F39F6]">${totalAmount}</span></h3>
                            </div>
                            <button 
                                onClick={handleCheckout} 
                                className="btn btn-lg px-12 bg-gradient-to-r from-[#4F39F6] to-[#9514FA] text-white border-none rounded-2xl shadow-xl hover:scale-105 transition-transform"
                            >
                                Proceed to Checkout
                            </button>
                        </div>
